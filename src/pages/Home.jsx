    ...
    <div className="min-h-screen bg-gray-100 p-3 flex flex-col gap-3 font-sans text-gray-800">
      ...
      <nav className="bg-blue-900 text-white px-8 py-5 flex justify-between items-center w-full rounded-t-lg shadow-md">
        ...
        <div className="hidden md:flex gap-8 text-lg font-medium items-center">
          ...
          <Link to="/matches" className="font-bold hover:text-gray-300 transition">Matches</Link>
          <Link to="/about" className="hover:text-blue-300 transition">About</Link>

          {/*  Admin Inbox */}
          {token && role === 'admin' && (
            <Link to="/inbox" className="text-sm bg-yellow-500 text-black px-3 py-1.5 rounded-lg font-bold hover:bg-yellow-400 transition flex items-center gap-1">
              📩 Inbox
            </Link>
          )}
        </div>
        ...
        <div className="flex gap-6 items-center">
          ...
          {role !== 'admin' && (
            <Link to="/contact" className="hover:text-blue-300 hidden lg:block transition font-medium">
              Contact
            </Link>
          )}
      ...
      <main className="bg-[#f0fdf4] flex-1 flex flex-col items-center relative min-h-[550px] w-full rounded-lg shadow-inner overflow-hidden">
        ...
        <div className="flex flex-col items-center justify-center flex-1 mt-8 z-10 text-center px-4 pb-24">
        ...
        <div className="absolute bottom-12 w-full px-6 flex flex-col md:flex-row justify-center gap-6 max-w-4xl mx-auto z-10">
          ...
          <Link to="/live-matches" className="bg-white py-4 px-6 w-full md:w-64 text-center text-lg font-bold text-blue-900 rounded-xl shadow-md cursor-pointer hover:shadow-xl transition duration-300 border-b-4 border-blue-500 flex items-center justify-center gap-2 hover:bg-blue-50 transform hover:-translate-y-1">
            <span className="w-3 h-3 bg-red-500 rounded-full animate-pulse"></span> Live Matches
          </Link>

          {(role === 'admin' || role === 'scorer') && (
            <Link to="/mycricket" className="bg-white py-4 px-6 w-full md:w-64 text-center text-lg font-bold text-blue-900 rounded-xl shadow-md cursor-pointer hover:shadow-xl transition duration-300 border-b-4 border-green-500 flex items-center justify-center gap-2 hover:bg-green-50 transform hover:-translate-y-1">
              🏏 My Cricket
            </Link>
          )}

          <a
            href="https://www.amazon.in/s?k=cricket+equipment"
            target="_blank"
            rel="noopener noreferrer"
            className="bg-white py-4 px-6 w-full md:w-64 text-center text-lg font-bold text-blue-900 rounded-xl shadow-md cursor-pointer hover:shadow-xl transition duration-300 border-b-4 border-yellow-500 flex items-center justify-center gap-2 hover:bg-yellow-50 transform hover:-translate-y-1"
          >
            🛒 Store
          </a>
        </div>
